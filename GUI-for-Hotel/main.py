...
#=================== Main Items (Hotel Food items) =======================
menu_category = ["Italian","American","Indian","French","Spanish","Japanese","Mediterranean", "Korean","Chinese"]

menu_category_dict = {"Italian":"1 Italian.txt","American":"2 American.txt",
                "Indian":"3 Indian.txt","French":"4 French.txt",
                "Spanish":"5 Spanish.txt","Japanese":"6 Japanese.txt",
                "Mediterranean":"7 Mediterranean.txt", "Korean":"8 Korean.txt",
                "Chinese":"9 Chinese.txt"}
...
def bill_button_operation():
    ...
    if ans=="yes":
        ...
        bill.wm_iconbitmap("ganesh.ico")
        bill_text_area = Text(bill, font=("arial", 12))
        st = "\t\t\t\tGanesh Hotel\n\t\t\tNeelam Nagar, Mumbai-400601\n"
        ...
        bill_day = str(t.tm_mday)
        bill_month = str(t.tm_mon)
        bill_year = str(t.tm_year)
        bill_time = str(t.tm_hour)+":"+str(t.tm_min)+":"+str(t.tm_sec)
        #Customer Name & Contact
        st += f"\nCustomer Name:- {customer_name}\nCustomer Bill No.:{bill_no.get()}\n"
        ...
        st += "-"*130
        st += "\n \t\t Enjoy your meal, Have a Good Day.. Thank you!!!\n"
        st += "-"*130 
        ...
        file = open(f"Bill Records\\{folder}\\{customer_name+bill_no.get()}.txt", "w")
        file.write(st)
        file.close()
        ...
        cur.execute('''INSERT INTO bill_data (name, bill_no, bill_day, bill_month, bill_year, bill_time, price)
                                VALUES (?, ?, ?, ?, ?, ?, ?)''', (str(customer_name),str(y),str(bill_day),str(bill_month),str(bill_year),str(bill_time),str(bill_price)))
        conn.commit()
        info = "SELECT * FROM bill_data"
        for row in cur.execute(info):
...
root.title("Welcome to Ganesh Hotel")
root.wm_iconbitmap("ganesh.ico")
...
title_label = Label(title_frame, text="Ganesh Hotel", 
                    font=("times new roman", 20, "bold"),bg = "#074463", fg="white", pady=5)
title_label.pack()

#==============Customer=============
customer_frame = LabelFrame(root,text="Customer Details",font=("times new roman", 15, "bold"),
                            bd=8, bg="#074463", fg="Gold", relief=GROOVE)
...
customer_bill_label = Label(customer_frame, text="Bill No", 
                    font=("arial", 15, "bold"),bg = "#074463", fg="white")
customer_bill_label.grid(row = 0, column = 2)

rand_bill_no = StringVar()
rand_bill_no.set("")
customer_bill_entry = Entry(customer_frame, width=20,font="arial 15",bd=5, state=DISABLED,textvariable= rand_bill_no)
customer_bill_entry.grid(row=0,column=4, padx=50)
...
menu_label = Label(menu_frame, text="Menu", 
                    font=("times new roman", 20, "bold"),bg = "#074463", fg="gold", pady=0)
...
combo_menu.grid(row=0,column=1,padx=20)

show_button = Button(menu_category_frame, text="Search", font = 'Times 14 bold',bg = 'cadet blue', fg ='White',bd = 5,padx=10,
                                          width=10,activebackground = 'gold',
                        command=show_button_operation)
show_button.grid(row=0,column=2,padx=20)

show_all_button = Button(menu_category_frame, text="Show All", font = 'Times 14 bold',bg = 'cadet blue', fg ='White',bd = 5,padx=10,
                                          width=10,activebackground = 'gold',
                        command=load_menu)
...
item_title_label = Label(item_frame, text="Item", 
                    font=("times new roman", 20, "bold"),bg = "#074463", fg="gold")
...
add_button = Button(item_frame3, text="Add Item", font = 'Times 14 bold',bg = 'cadet blue', fg ='White',bd = 5,padx=10,
                                          width=10,activebackground = 'gold'
                        ,command=add_button_operation)
add_button.grid(row=0,column=0,padx=10,pady=30)

remove_button = Button(item_frame3, text="Remove Item", font = 'Times 14 bold',bg = 'cadet blue', fg ='White',bd = 5,padx=10,
                                          width=10,activebackground = 'gold'
                        ,command=remove_button_operation)
remove_button.grid(row=0,column=1,padx=10,pady=30)

update_button = Button(item_frame3, text="Update Quantity",font = 'Times 14 bold',bg = 'cadet blue', fg ='White',bd = 5,padx=10,
                                          width=10,activebackground = 'gold',command=update_button_operation)
update_button.grid(row=0,column=2,padx=10,pady=30)

clear_button = Button(item_frame3, text = 'Clear All', font = 'Times 14 bold',bg = 'cadet blue', fg ='White',bd = 5,padx=10,
                                          width=10,activebackground = 'gold', command = clear_button_operation)
clear_button.grid(row=0,column =3,padx=0,pady=0) 
...
bill_button = Button(order_frame, text = 'Generate Bill>>', font = 'Times 14 bold',bg = 'cadet blue', fg ='White',bd = 5,padx=20,
                                          width=10,activebackground = 'gold', command = bill_button_operation)
bill_button.pack(side=LEFT,anchor=SW,padx=25,pady=3)


cancel_button = Button(order_frame, text = 'Cancel Order ❌', font = 'Times 14 bold',bg = 'cadet blue', fg ='White',bd = 5,padx=20,
                                          width=12,activebackground = 'gold', command = cancel_button_operation)
cancel_button.pack(side=LEFT,anchor=SW,padx=25,pady=3)
